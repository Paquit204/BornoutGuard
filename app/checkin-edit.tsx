...
export default function CheckinEditScreen() {
  ...
  useEffect(() => {
    if (loaded || !params.checkin) return;
    try {
      const data = JSON.parse(params.checkin as string);
      const today = new Date().toDateString();
      const entryDateObj = new Date(data.created_at);
      
      if (today !== entryDateObj.toDateString()) {
        setEntryDate(entryDateObj.toLocaleDateString('en-US', { 
          weekday: 'long', 
          year: 'numeric', 
          month: 'long', 
          day: 'numeric' 
  ...
  const handleSave = async () => {
    ...
    console.log('📝 Updating check-in:', updates);
    ...
    return (
      <View style={styles.root}>
        ...
        <View style={styles.blockedContainer}>
          ...
          <Text style={styles.blockedTitle}>You cannot edit this check‑in</Text>
          {entryDate && <Text style={styles.blockedDate}>Date: {entryDate}</Text>}
          <Text style={styles.blockedDescription}>Only today’s check‑in can be modified.</Text>
      ...
      <ScrollView style={styles.container} contentContainerStyle={[styles.content, { paddingBottom: 90 }]}>
        ...
        {entryDate && (
          <View style={styles.dateDisplay}>
            ...
            <Text style={styles.dateValue}>{entryDate}</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  blockedTitle: { ...Typography.heading, textAlign: 'center', color: Colors.danger, marginBottom: Spacing.sm },
  blockedDate: { ...Typography.body, fontWeight: '500', color: Colors.textPrimary, marginBottom: Spacing.sm },
  blockedDescription: { ...Typography.body, textAlign: 'center', color: Colors.textSecondary, marginBottom: Spacing.xl },
  goBackButton: { backgroundColor: Colors.primary, paddingVertical: 12, paddingHorizontal: 32, borderRadius: 12 },
  ...
  dateDisplay: { flexDirection: 'row', alignItems: 'center', marginBottom: Spacing.md, backgroundColor: Colors.border, paddingHorizontal: Spacing.md, paddingVertical: Spacing.sm, borderRadius: 10 },
  dateLabel: { ...Typography.small, fontWeight: '600', color: Colors.textSecondary, marginRight: Spacing.sm },
  dateValue: { ...Typography.body, fontWeight: '500', color: Colors.textPrimary, flex: 1 },
  ...
  moodBtn: { alignItems: 'center', paddingVertical: Spacing.sm, paddingHorizontal: Spacing.xs, borderRadius: 10, backgroundColor: Colors.border, minWidth: 52 },
  ...
  previewCard: { backgroundColor: Colors.card, borderRadius: 16, padding: Spacing.lg, alignItems: 'center', ...Shadows.card, marginBottom: Spacing.md },
